...
class Game:
    ...
    def prior_start(self):
        ...
        self.start_button = tk.Button(master=player_frame, text=" - Click to Begin - ", command=lambda: self.initial_start(self.start_button), font=("Verdana", 28, "bold"),borderwidth=8,relief="raised")
        ...
        if self.remaining_players > 1:
            ...
            if self.players[self.player_index].position == 1:
                ...
                if (player_human.truth_count > 0):
                    ...
                    self.button_player_t = tk.Button(master=player_frame, text="TRUTH", font=("Verdana", 12, "bold"), command=lambda: self.player_truth(player_human),borderwidth=8,relief="raised")
                    self.button_player_t.place(relx=0.3, rely=0.5, anchor="center")

                if (player_human.lie_count > 0):
                    # Add a button to the player_frame
                    self.button_player_l = tk.Button(master=player_frame, text="LIE", font=("Verdana", 12, "bold"), command=lambda: self.player_lie(player_human),borderwidth=8,relief="raised")
                    self.button_player_l.place(relx=0.5, rely=0.5, anchor="center")   
                
                if self.round_turns != 0:
                # Add a button to the player_frame
                    self.button_player_c = tk.Button(master=player_frame, text="CALL", font=("Verdana", 12, "bold"), command=lambda: self.player_call(player_human),borderwidth=8,relief="raised")
                ...
                self.btn_decrease = tk.Button(master=player_frame, text="-", font=("Verdana", 12, "bold"), command=self.decrease,borderwidth=8,relief="raised")
                self.btn_decrease.place(relx=0.42, rely=0.9, anchor="center")

                self.btn_increase = tk.Button(master=player_frame, text="+", font=("Verdana", 12, "bold"), command=self.increase,borderwidth=8,relief="raised")
    ...
    def prior_update_table_ai(self):
        ...
        if not self.next:
            self.btn_next= tk.Button(master=player_frame, text="[next]", font=("Verdana", 14, "bold"), command=self.update_table_ai,borderwidth=8,relief="raised")
            self.btn_next.place(relx=0.85, rely=0.8, anchor="center")
            self.next = True
        ...
        if self.order_index == 0 and self.players[self.order_list[self.order_index]].position == 1:
            ...
            if self.ai_decision == "play":
                ...
                if self.player_com.position == 2:
                    ...
                    self.lbl_played_COM1["text"] = f"Played {self.table.get_lasthand_len()} X cards"
                ...
                if self.player_com.position == 3:
                    ...
                    self.lbl_played_COM2["text"] = f"Played {self.table.get_lasthand_len()} X cards "
                ...
                if self.player_com.position == 4:
                    ...
                    self.lbl_played_COM3["text"] = f"Played {self.table.get_lasthand_len()} X cards"
                ...
                self.call_output(self.player_com)   
    ...
    def player_truth(self, player):
        if not self.confirmed:
            self.btn_confirm= tk.Button(master=player_frame, text="[confirm]", font=("Verdana", 14, "bold"), command=lambda: self.update_table_player(player),borderwidth=8,relief="raised")
    ...
    def call_output(self, player):
        ...
        if (self.order_index == 0):
            temp_order_index = len(self.order_list)-1
            print(f"CALL OUTPUT IF. PLAYER ::: temp_order_index = {temp_order_index}")

            temp_player_index = self.order_list[temp_order_index]
            previous_player = self.players[temp_player_index]
        else:
            temp_order_index = self.order_index - 1
            temp_player_index = self.order_list[temp_order_index]
            previous_player = self.players[temp_player_index]
    ...
    def stop_music(self):
        ...
        self.boss_sound.stop() # doesn't work

    def win_screen(self):
        self.stop_music()
        self.win_label_flash()

        window.after(1500, self.play_win_sound)
